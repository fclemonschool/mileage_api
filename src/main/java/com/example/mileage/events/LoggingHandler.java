...
public class LoggingHandler {
  ...
  /**
   * com.example.mileage.controller 이하 패키지의 모든 클래스 이하 모든 메서드 호출 전에 적용.
   *
  ...
  /**
   * com.example.mileage.controller 이하 패키지의 모든 클래스 이하 모든 메서드 리턴 후에 적용.
   *
   * @param joinPoint 발생 지점
   * @param returnObj 메서드 리턴 값
   */
  @AfterReturning(value = "execution(* com.example.mileage.controller.*.*(..))",
      returning = "returnObj")
  ...
  /**
   * com.example.mileage.service 이하 패키지의 모든 클래스에서 오류 발생 시 적용.
   *
   * @param joinPoint 발생 지점
   * @param exception 오류 정보
   */
  @AfterThrowing(pointcut = "execution(* com.example.mileage.service.*.*(..))",
      throwing = "exception")
